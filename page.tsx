...
// ▇▇▇ Tool Detection (client-side, for quick tools) ▇▇▇
...
export default function ChatPage() {
  ...
  const persistMessages = useCallback((msgs: ChatMessage[], convId: string | null, model: string) => {
    const chatMsgs = msgs.filter(m => m.role !== 'system').map(m => ({ role: m.role as 'user' | 'assistant', content: m.content }))
  ...
  const handleSendMessage = useCallback(async (text?: string) => {
    ...
    // Detect tools (client-side quick tools)
    ...
    try {
      ...
      // Persist
      persistMessages(allMessages, activeConvId, selectedModel)
    } catch {
      msgIdCounter.current++
      ...
      const allMessages = [...updatedMessages, errMsg]
      setMessages(allMessages)
    } finally {
      setIsLoading(false)
      inputRef.current?.focus()
    }
  }, [input, isLoading, messages, selectedModel, temperature, maxTokens, systemPrompt, activeConvId, persistMessages])
      ...
      <header className="flex items-center justify-between h-14 px-4 border-b border-border/50 bg-background/80 backdrop-blur-xl shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {settingsOpen ? <ChevronRight className="w-3 h-3" /> : <ChevronLeft className="w-3 h-3" />}
          </Button>
          ...
          <aside className="w-64 shrink-0 border-r border-border/50 bg-muted/20 overflow-y-auto hidden md:block">
            <div className="p-3">
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2.5 text-sm font-medium rounded-lg border border-dashed border-border hover:bg-muted transition-colors mb-2"
              >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                        <Clock className="w-2.5 h-2.5" />{new Date(conv.updatedAt).toLocaleDateString()}
                      </p>
                    </div>
                    <button
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
            ...
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm text-muted-foreground mt-2 max-w-sm">
                  Free multi-model AI chat. Conversations are saved automatically. Try a quick action or type your message below.
                </p>
          ...
          <div className="shrink-0 border-t border-border/50 bg-background/80 backdrop-blur-xl p-4">
            ...
            <p className="text-center text-[11px] text-muted-foreground mt-2">
              Powered by Qwen 2.5 via HuggingFace Inference API | Conversations saved locally
            </p>
          </div>
        </main>
      </div>
